##############################################################################
# Script information                                                      
# Title: Conditional cis-eQTL mapping for CD4 NC cells - Round 2
# Author: Seyhan Yazar
# Date: 2020-12-23
# Description: This R script was written to run an array job per chromosome 
# for five subsets using "round2.run_spearman_rank_test.sh" script
##############################################################################
...
# Input filenames
r1_residual_filename <- sprintf("round1/percent%s_chr%s_log_residuals.tsv",percent,chrNumber)
r1_significant_SNPs <- sprintf("round1/percent%s_chr%s_significant_correlation_results.tsv",percent,chrNumber)
...
fwrite(eSNP1,sprintf("round1/percent%s_chr%s_lead_eSNP.tsv",percent,chrNumber),sep="\t",quote=F)
...
fwrite(adjusted_residual_df,sprintf("round2/percent%s_chr%s_eSNP1_adjusted_residuals.tsv",percent,chrNumber),sep="\t",quote=F)
...
fwrite(adjusted_spearman_df,sprintf("round2/percent%s_chr%s_round2_correlation_results.tsv",percent,chrNumber),sep="\t",quote=F)
...
fwrite(adjusted_spearman_df_significant,sprintf("round2/percent%s_chr%s_round2_significant_correlation_results.tsv",percent,chrNumber),sep="\t",quote=F)
