##############################################################################
# Script information                                                      
# Title: Conditional cis-eQTL mapping for CD4 NC cells - Round 3
# Author: Seyhan Yazar
# Date: 2020-12-23
# Description: This R script was written to run an array job per chromosome 
# for five subsets using "round3.run_spearman_rank_test.sh" script
##############################################################################
...
# Input filenames
r2_residual_filename <- sprintf("round2/percent%s_chr%s_eSNP1_adjusted_residuals.tsv", percent, chrNumber)
r2_significant_SNPs <- sprintf("round2/percent%s_chr%s_round2_significant_correlation_results.tsv", percent, chrNumber)
...
fwrite(eSNP2,sprintf("round2/percent%s_chr%s_eSNP2.tsv",percent,chrNumber),sep="\t",quote=F)
...
fwrite(adjusted_residual_df,sprintf("round3/percent%s_chr%s_eSNP2_adjusted_residuals.tsv",percent,chrNumber),sep="\t",quote=F)
...
fwrite(adjusted_spearman_df, sprintf("round3/percent%s_chr%s_round3_correlation_results.tsv", percent, chrNumber), sep="\t", quote=F)
...
fwrite(adjusted_spearman_df_significant, sprintf("round3/percent%s_chr%s_round3_significant_correlation_results.tsv", percent, chrNumber), sep="\t", quote=F)
