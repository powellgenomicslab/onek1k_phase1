##############################################################################
# Script information                                                      
# Title: Conditional cis-eQTL mapping for CD4 NC cells - Round 4
# Author: Seyhan Yazar
# Date: 2020-12-23
# Description: This R script was written to run an array job per chromosome 
# for five subsets using "round4.run_spearman_rank_test.sh" script
##############################################################################
...
# Input filenames
r3_residual_filename <- sprintf("round3/percent%s_chr%s_eSNP2_adjusted_residuals.tsv", percent, chrNumber)
r3_significant_SNPs <- sprintf("round3/percent%s_chr%s_round3_significant_correlation_results.tsv", percent, chrNumber)
...
fwrite(eSNP3,sprintf("round3/percent%s_chr%s_eSNP3.tsv",percent,chrNumber),sep="\t", quote=F)
...
fwrite(adjusted_residual_df,sprintf("round4/percent%s_chr%s_eSNP3_adjusted_residuals.tsv", percent, chrNumber), sep="\t", quote=F)
...
fwrite(adjusted_spearman_df, sprintf("round4/percent%s_chr%s_round4_correlation_results.tsv", percent, chrNumber), sep="\t", quote=F)
...
fwrite(adjusted_spearman_df_significant, sprintf("round4/percent%s_chr%s_round4_significant_correlation_results.tsv", percent, chrNumber), sep="\t", quote=F)
