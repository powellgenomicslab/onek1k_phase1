...
# per chromosome for 14 cell types using "round3.run_spearman_rank_test.R" script. 
...
CTYPEFILE="/onek1k/Scripts/single_cell_cis_eQTL_mapping/$1"

SAMPLE=`head -n $SGE_TASK_ID $CTYPEFILE | tail -n 1 | awk '{print $1}'`
CELLLABEL=`head -n $SGE_TASK_ID $CTYPEFILE | tail -n 1 | awk '{print $2}'`

if [ ! -d ${CELLLABEL}/round3 ]
then
     mkdir ${CELLLABEL}/round3
else
     echo "Directory exists"
fi

# Main script file
RSCRIPT="/onek1k/Scripts/single_cell_cis_eQTL_mapping/round3.run_spearman_rank_test.R"

# Do the main job
$R_PATH/Rscript --verbose ${RSCRIPT} ${CELLLABEL} ${SAMPLE} > rout/round3_${CELLLABEL}_${SAMPLE}.Rout
